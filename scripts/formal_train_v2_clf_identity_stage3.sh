...
[[ -f "$FORMAL_PARENT_CHECKPOINT" && -f "$FORMAL_PARENT_FINALIZED_MANIFEST" ]] || {
  echo "explicit validated Stage 2 parent is required" >&2; exit 1;
}
...
FINAL_CHECKPOINT="$FORMAL_CHECKPOINT_DIR/step-10000.ckpt"
...
PYTHON="$PYTHON" "$ROOT/scripts/run_with_durable_log.sh" \
  ...
  "$PYTHON" -I -B "$ROOT/scripts/run_exact_tabicl.py" \
    ...
    --formal_training true --formal_stage stage3 \
    ...
    --gradient_clipping 1 --prior_type graph_scm --prior_device cpu --n_jobs 16 \
    --batch_size_per_gp 1 --min_features 1 --max_features 100 --max_classes 10 \
    --min_seq_len 400 --max_seq_len 60000 --log_seq_len true --replay_small false \
...
PYTHON="$PYTHON" "$ROOT/scripts/run_with_durable_log.sh" \
  ...
  "$PYTHON" -I -B "$ROOT/scripts/run_exact_tabicl.py" \
    ...
    --study-id "$FORMAL_STUDY_ID" --parent-stage stage2 \
...
"$PYTHON" -I -B "$ROOT/scripts/prune_identity_stage_checkpoints.py" \
  --checkpoint-dir "$FORMAL_CHECKPOINT_DIR" --terminal-step 10000 \
